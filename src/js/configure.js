...
module.exports = {
  ...
  getHOSxPConnectionPool() {
    let HOS_HOST = config.get('HOS_HOST') || '172.16.0.3';
    let HOS_DBNAME = config.get('HOS_DBNAME') || 'hos';
    let HOS_USER = config.get('HOS_USER') || 'sa';
    let HOS_PASSWORD = config.get('HOS_PASSWORD') || '0437891241';
  ...
  getHDCConnectionPool() {
    let HDC_HOST = config.get('HDC_HOST') || '172.16.0.6';
    let HDC_DBNAME = config.get('HDC_DBNAME') || 'hdc';
    let HDC_USER = config.get('HDC_USER') || 'hdc';
    let HDC_PASSWORD = config.get('HDC_PASSWORD') || '043789124';
